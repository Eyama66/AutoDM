...
import {
  ...
  type ModulePlotLike,
  type PlotFrontier,
} from "../engine/campaignPlotUtils.js";
...
export interface PossibilitySpace {
  deployableEncounters: { id: string; triggerHint?: string }[];
  advancablePlotNodes: { id: string; title: string; revealHint?: string }[];
  presentNpcs: { name: string; canSpeak: boolean; canBeInteracted: boolean }[];
...
export function buildPromptPacket(context: PromptContext) {
  const state = context.playerState;
  const characterSheet = state?.characterSheet;
  const plotFrontier =
    context.plotFrontier ||
    buildPlotFrontier(context.modulePlot, state?.plotProgress);
  ...
  return {
    module: {
      ...
      plot: {
        completed: plotFrontier.completedNodeIds,
        active: plotFrontier.activeNodeIds,
        allowedNext: plotFrontier.allowedNodeIds,
      },
    },
    state: {
    ...
    scene: {
      ...
      allowedNpcSpeakers: (context.allowedNpcSpeakerNames || []).filter(Boolean),
      dm: {
